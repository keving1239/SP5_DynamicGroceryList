...
import {Text,StyleSheet, View, TouchableOpacity, KeyboardAvoidingView, TextInput, ScrollView, Keyboard, Alert, Div, Nav,li, ul, prompt} from 'react-native';
...
const Notes = ({navigation, ...props}) => {
    ...
    async function db_share(item){
        const call = await Axios.post("http://10.0.0.26/share_note" , {user_id: props.userID, note_id: item}).catch((err) => console.log(err));
        return  call.data;
    }
   ...
   async function shareNote(item){
    ...
    function deleteNote(index){
        ...
        'This will send the note to the Trash',
        [
            {
                text: 'Yes',
                onPress: () =>{
    ...
    function clear_all(){
        Alert.alert('Are you sure you want to clear all notes?',
        'This will send all notes to the Trash',
        [
            {
                text: 'Yes',
                onPress: () =>{
